...
Un SIEM (Security Information and Event Management) est essentiel dans la conduite d'une politique de sécurité informatique. C'est un outil qui va rassembler des données depuis différentes machines du système d'information, les centraliser et effectuer des analyses sur elles. Chaque appareil du réseau va générer des logs que l'on peut regrouper en deux types:
- des logs à propos d'événements survenus à l'intérieur de la machine hôte. Par exemple un utilisateur qui accède à un fichier ou une log d'un processus s'exécutant.
- des logs générés quand des machines communiquent entre elles ou avec l'extérieur. Par exemple du trafic web sur le site www.insee.fr, un fichier récupéré via le protocole FTP ou encore un utilisateur se connectant au moyen d'un VPN.


Importance of SIEM
Shows SOC CapabilitiesNow that we have covered various types of logs, it's time to understand the importance of SIEM. As all these devices generate hundreds of events per second, examining the imglogs on each device one by one in case of any incident can be a tedious task. That is one of the advantages of having a SIEM solution in place. It not only takes logs from various sources in real-time but also provides the ability to correlate between events, search through the logs, investigate incidents and respond promptly. Some key features provided by SIEM are:
